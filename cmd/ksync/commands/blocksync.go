...
func init() {
	...
	blockSyncCmd.Flags().Int64Var(&backupInterval, "backup-interval", 0, "block interval to write backups of data directory (set 0 to disable backups)")
	blockSyncCmd.Flags().Int64Var(&backupKeepRecent, "backup-keep-recent", 0, "number of latest backups to be keep (0 to keep all backups)")
...
var blockSyncCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		if err != nil {
			panic(fmt.Errorf("failed to create backup config: %w", err))
		}
