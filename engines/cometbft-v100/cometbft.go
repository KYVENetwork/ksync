...
import (
	...
	tmProtoState "github.com/KYVENetwork/cometbft/v100/api/cometbft/state/v1"
	...
	cometP2P "github.com/KYVENetwork/cometbft/v100/p2p"
	...
	tmState "github.com/KYVENetwork/cometbft/v100/state"
	tmStore "github.com/KYVENetwork/cometbft/v100/store"
	tmTypes "github.com/KYVENetwork/cometbft/v100/types"
...
func (engine *Engine) GetChainId() (string, error) {
	...
	_, genDoc, err := nm.LoadStateFromDBOrGenesisDocProvider(engine.stateDB, defaultDocProvider, engine.config.Storage.GenesisHash)
	if err != nil {
...
func (engine *Engine) DoHandshake() error {
	defaultDocProvider := nm.DefaultGenesisDocProviderFunc(engine.config)
	state, genDoc, err := nm.LoadStateFromDBOrGenesisDocProvider(engine.stateDB, defaultDocProvider, engine.config.Storage.GenesisHash)
	if err != nil {
		return fmt.Errorf("failed to load state and genDoc: %w", err)
	}
...
func (engine *Engine) ApplyBlock(runtime *string, value []byte) error {
	...
	state, err := engine.blockExecutor.ApplyBlock(engine.state, blockId, engine.prevBlock, engine.prevBlock.Height+1)
	if err != nil {
...
func (engine *Engine) ApplyFirstBlockOverP2P(runtime string, value, nextValue []byte) error {
	...
	genDoc, err := tmTypes.GenesisDocFromFile(engine.config.Genesis)
	if err != nil {
...
func (engine *Engine) GetGenesisHeight() (int64, error) {
	genDoc, err := tmTypes.GenesisDocFromFile(engine.config.Genesis)
	if err != nil {
		return 0, fmt.Errorf("failed to read genesis file: %w", err)
	}
...
func (engine *Engine) GetAppHeight() (int64, error) {
	...
	info, err := socketClient.Info(context.Background(), &v1.InfoRequest{})
	if err != nil {
...
func (engine *Engine) GetSnapshots() ([]byte, error) {
	...
	res, err := socketClient.ListSnapshots(context.Background(), &v1.ListSnapshotsRequest{})
	if err != nil {
...
func (engine *Engine) GetSnapshotChunk(height, format, chunk int64) ([]byte, error) {
	...
	res, err := socketClient.LoadSnapshotChunk(context.Background(), &v1.LoadSnapshotChunkRequest{
...
func (engine *Engine) GetBlock(height int64) ([]byte, error) {
	block, _ := engine.blockStore.LoadBlock(height)
...
func (engine *Engine) GetState(height int64) ([]byte, error) {
	...
	lastBlock, _ := engine.blockStore.LoadBlock(height)
	currentBlock, _ := engine.blockStore.LoadBlock(height + 1)
	nextBlock, _ := engine.blockStore.LoadBlock(height + 2)
...
func (engine *Engine) GetSeenCommit(height int64) ([]byte, error) {
	block, _ := engine.blockStore.LoadBlock(height + 1)
...
func (engine *Engine) OfferSnapshot(value []byte) (string, uint32, error) {
	...
	if err := json.Unmarshal(value, &bundle); err != nil {
		return abciTypes.OFFER_SNAPSHOT_RESULT_UNKNOWN.String(), 0, fmt.Errorf("failed to unmarshal tendermint-ssync bundle: %w", err)
	}

	socketClient := abciClient.NewSocketClient(engine.config.ProxyApp, false)

	if err := socketClient.Start(); err != nil {
		return abciTypes.OFFER_SNAPSHOT_RESULT_UNKNOWN.String(), 0, fmt.Errorf("failed to start socket client: %w", err)
	}

	res, err := socketClient.OfferSnapshot(context.Background(), &v1.OfferSnapshotRequest{
		Snapshot: bundle[0].Value.Snapshot,
		AppHash:  bundle[0].Value.State.AppHash,
	})

	if err != nil {
		return abciTypes.OFFER_SNAPSHOT_RESULT_UNKNOWN.String(), 0, err
	}

	if err := socketClient.Stop(); err != nil {
		return abciTypes.OFFER_SNAPSHOT_RESULT_UNKNOWN.String(), 0, fmt.Errorf("failed to stop socket client: %w", err)
	}
...
func (engine *Engine) ApplySnapshotChunk(chunkIndex uint32, value []byte) (string, error) {
	...
	if err := json.Unmarshal(value, &bundle); err != nil {
		return abciTypes.APPLY_SNAPSHOT_CHUNK_RESULT_UNKNOWN.String(), fmt.Errorf("failed to unmarshal tendermint-ssync bundle: %w", err)
	}

	nodeKey, err := p2p.LoadNodeKey(engine.config.NodeKeyFile())
	if err != nil {
		return abciTypes.APPLY_SNAPSHOT_CHUNK_RESULT_UNKNOWN.String(), fmt.Errorf("loading node key file failed: %w", err)
	}

	socketClient := abciClient.NewSocketClient(engine.config.ProxyApp, false)

	if err := socketClient.Start(); err != nil {
		return abciTypes.APPLY_SNAPSHOT_CHUNK_RESULT_UNKNOWN.String(), fmt.Errorf("failed to start socket client: %w", err)
	}

	res, err := socketClient.ApplySnapshotChunk(context.Background(), &v1.ApplySnapshotChunkRequest{
		Index:  chunkIndex,
	...
	if err != nil {
		return abciTypes.APPLY_SNAPSHOT_CHUNK_RESULT_UNKNOWN.String(), err
	}

	if err := socketClient.Stop(); err != nil {
		return abciTypes.APPLY_SNAPSHOT_CHUNK_RESULT_UNKNOWN.String(), fmt.Errorf("failed to stop socket client: %w", err)
	}
...
func (engine *Engine) PruneBlocks(toHeight int64) error {
	...
	if toHeight > base {
		if _, err := engine.stateStore.PruneStates(base, toHeight, toHeight, 0); err != nil {
