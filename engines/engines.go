...
func EngineSourceFactory(engine, homePath, registryUrl, source string, continuationHeight int64) types.Engine {
	// if the engine was specified by the user or the source is empty we determine the engine by the engine input
	if engine != "" || source == "" {
		return EngineFactory(engine, homePath)
	}
...
func EngineFactory(engine, homePath string) types.Engine {
	switch engine {
	case "":
		return &cometbft_v38.Engine{HomePath: homePath}
	case utils.EngineTendermintV34:
		return &tendermint_v34.Engine{HomePath: homePath}
	case utils.EngineCometBFTV37:
		return &cometbft_v37.Engine{HomePath: homePath}
	case utils.EngineCometBFTV38:
		return &cometbft_v38.Engine{HomePath: homePath}
	case utils.EngineCelestiaCoreV34:
		return &celestia_core_v34.Engine{HomePath: homePath}
