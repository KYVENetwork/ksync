...
func StartSyncProcess(s *types.SyncProcess, home string, poolId int64, restEndpoint string, targetHeight int64, apiServer bool) {
	if !s.Running {
		s.Running = true
		if s.Name == "p2p" {
			...
			go db.StartDBExecutor(s.QuitCh, home, poolId, restEndpoint, targetHeight, apiServer)
		}
	}
}
