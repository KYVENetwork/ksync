...
func StartBinaryProcessForDB(engine types.Engine, binaryPath string, debug bool, args []string) (*exec.Cmd, error) {
	if binaryPath == "" {
		return nil, nil
	}
...
func StartBinaryProcessForP2P(engine types.Engine, binaryPath string, debug bool, args []string) (*exec.Cmd, error) {
	if binaryPath == "" {
		return nil, nil
	}
