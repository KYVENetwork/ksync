...
func StartStateSyncExecutor(app *app.CosmosApp, snapshotCollector types.SnapshotCollector, snapshotHeight int64) error {
	...
	logger.Info().Msgf("offering snapshot for height %d: ACCEPT", snapshot.Height)
	...
	logger.Info().Msgf("applied snapshot chunk %d/%d: ACCEPT", 1, snapshot.Chunks)
	...
	for chunkIndex := int64(1); chunkIndex < int64(snapshot.Chunks); chunkIndex++ {
		...
		logger.Info().Msgf("downloaded snapshot chunk %d/%d", chunkIndex+1, snapshot.Chunks)
		...
		logger.Info().Msgf("applied snapshot chunk %d/%d: ACCEPT", chunkIndex+1, snapshot.Chunks)
	}
	...
	logger.Info().Uint64("height", snapshot.Height).Uint32("format", snapshot.Format).Str("hash", fmt.Sprintf("%X", snapshot.Hash)).Msg("snapshot restored")
	return nil
}
